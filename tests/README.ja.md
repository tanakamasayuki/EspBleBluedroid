...
| `peer/hid_keyboard_host` | HID Host側を本ライブラリのkeyboard device相手に検証。デバイスが何だったかを報告するdiscovery（keyboardのReport ID、HID Informationのcountry code、Output Reportの有無、battery level）、キー入力がusageとlayout適用後の文字にdecodeされること、状態全体のeventと差分のevent、HostのLED write、切断でdiscovery済みhandleが残らないこと |
| `peer/hid_boot_protocol` | NKRO keyboardでのHID over GATT Boot Protocol。Boot Protocolが追加する2 characteristic、Protocol Modeの既定値と切り替え、同じusageがmodeに応じて29 byteのbitmapか固定8 byteのboot reportとして出ること、6キー超でのHID rolloverコード、boot LED write、選択中modeが使うreportのCCCDに`ready()`が従うこと |
