...
| GATT server | Two characteristics with the same UUID in one service; `add*Listener()` multi-listener registration | The wrapper looks characteristics up by UUID within a service, so a duplicate is unaddressable; events hold a single callback | [Gatt/Basics/Server](Gatt/Basics/Server/) |
| Profiles | HID over GATT (`hidKeyboard()`, `hidMouse()`, `hidHost()`, …), BLE MIDI | Not implemented in this library | EspBle's `Hid/` and `Midi/` examples have no counterpart here |
