...
| Profiles | The HID **host** side (`hidHost()`) | Only the device side is implemented; the report-map parser is already here, so the host is next (see [docs/STATUS.ja.md](../docs/STATUS.ja.md)) | EspBle's `Hid/KeyboardHost`, `Hid/VendorHost` and `Hid/CustomClient` have no counterpart here. Every HID **device** profile and BLE MIDI do: [Hid/](Hid/), [Midi/](Midi/) |
