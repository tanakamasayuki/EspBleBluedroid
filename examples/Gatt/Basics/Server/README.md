...
- **Two characteristics with the same UUID in one service are rejected here.** `addCharacteristic()` fails with `InvalidArgument` for the duplicate. The spec permits duplicates (HID Reports are the everyday case), but the Arduino-ESP32 Bluedroid wrapper looks characteristics up by UUID inside a service, so a duplicate could not be addressed reliably. Give each characteristic its own UUID, or split them across services.
...
- **A value larger than one ATT response depends on the client.** A client that performs Read Long gets the whole value; this library's own GATT client does not ([Gatt/Basics/Client](../Client/)), so it would see the value truncated. Keep server values within `mtu - 1` bytes when the peer might be an EspBleBluedroid central.
